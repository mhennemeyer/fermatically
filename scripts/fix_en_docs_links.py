#!/usr/bin/env python3
"""Fixes internal links in docs/*.en.md files from topics/-style to docs/-style paths."""
import re
import os
...
def fix_link(match, doc_rel_path):
    ...
    # Pattern: ../../<topic>/<article>/article_en.md or ../<article>/article_en.md
    m = re.match(r'(?:\.\./)*(\S+?)/article_en\.md', link_path)
    if not m:
        return full
    ...
    if '/' not in slug:
        # The .en.md file path tells us the series
        # e.g. grundlagen/elementare-zahlentheorie/01-was-ist-flt.en.md
        base_path = doc_rel_path.replace('.en.md', '.md')
        from_dir = os.path.dirname(base_path)
...
def fix_file(doc_rel_path):
    """Fix all article_en.md links in a single docs/*.en.md file."""
    filepath = os.path.join(DOCS, doc_rel_path)
    with open(filepath, 'r') as f:
        content = f.read()

    # Match markdown links containing article_en.md
    pattern = r'\(([^)]*article_en\.md)\)'
    ...
    if new_content != content:
        with open(filepath, 'w') as f:
            f.write(new_content)
        old_count = len(re.findall(r'article_en\.md', content))
        new_count = len(re.findall(r'article_en\.md', new_content))
        print(f"  ✅ {doc_rel_path}: {old_count - new_count} links fixed")
    else:
        if 'article_en.md' in content:
            print(f"  ⚠ {doc_rel_path}: links remain unchanged")


# Process all .en.md files in docs/
ALL_EN_FILES = []
for root, dirs, files in os.walk(DOCS):
    for f in files:
        if f.endswith('.en.md'):
            rel = os.path.relpath(os.path.join(root, f), DOCS)
            ALL_EN_FILES.append(rel)

for doc_file in sorted(ALL_EN_FILES):
    fix_file(doc_file)

print(f"\nDone. Processed {len(ALL_EN_FILES)} files.")
